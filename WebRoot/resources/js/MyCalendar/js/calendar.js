...
function drawCld(SY, SM) {
    ...
    for (i = 0; i < 42; i++) {
        ...
        if (sD > -1 && sD < cld.length) {  //日期内
            sObj.innerHTML = sD + 1;
            //wly 注册点击事件
						$("#GD" + i).unbind('click').click(function(){mOck(this,sD + 1,null);});
    ...
    for(var i = 0;i<hDays.length;i++){
		var ymdList = hDays[i].split('-');
		if(parseInt(ymdList[0]) == global.currYear && parseInt(ymdList[1]) == (global.currMonth+1)){
			var week = new Date(global.currYear, global.currMonth, 1, 0, 0, 0, 0).getDay();    //当月一日日期
			var gd = parseInt(ymdList[2]) + week - 1;
			$("#GD" + gd).addClass("selday")
			//hDays.push(list[i]);
		}
...
//日期点击函数    dateStr:"2017/03/21"
function mOck(thisObj, v,dateList){
	if(thisObj != null){
		...
		if(onoff == '0' && hDays.indexOf(dayJson)<0){
		...
		hDays = [];
		//var dateListStr = dateList.replace(/-/g,'/');
		var list = dateList.split(',');
		for(var i = 0;i<list.length;i++){
			hDays.push(list[i]);
			//delArry(hDays,dayJson);
			var ymdList = list[i].split('-');
			if(parseInt(ymdList[0]) == global.currYear && parseInt(ymdList[1]) == (global.currMonth+1)){
				var week = new Date(global.currYear, global.currMonth, 1, 0, 0, 0, 0).getDay();    //当月一号日期的星期
				var gd = parseInt(ymdList[2]) + week - 1;
				$("#GD" + gd).addClass("selday")
			}
